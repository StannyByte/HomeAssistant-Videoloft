...
from .storage import ApiKeyStore
...
class GeminiAPI:
    ...
    async def process_ai_search(self, selected_cameras: List[str]) -> None:
        ...
        Uses the persisted API key from server-side storage.
        """
        api_key = await ApiKeyStore(self.hass).async_get_key()
        if not api_key:
        ...
        for camera_uidd in selected_cameras:
            try:
                ...
                # Batch writes to reduce storage churn
                pending_writes = 0
        ...
        try:
            ...
            _LOGGER.warning(f"Final save of descriptions failed: {e}")
        _LOGGER.info(f"AI analysis completed: {events_processed} new events processed")

    async def get_gemini_description(self, image_bytes: bytes, api_key: str, max_retries: int = 3) -> Optional[str]:
        """Get description from Google Gemini API with retry logic and robust parsing."""
        for attempt in range(max_retries + 1):
            try:
                ...
                async with session.post(f"{url}?key={api_key}", json=data, timeout=timeout) as response:
                        if response.status == 200:
                            result = await response.json()
                            # Robustly extract description text
                            description = None
                            try:
                                ...
                                if not description and isinstance(result, dict) and "text" in result:
    ...
    async def _process_cameras_background(self, task_id: str, selected_cameras: List[str], 
        ...
        _LOGGER.info(f"AI analysis: processing {len(selected_cameras)} cameras; range {start_time_ms}..{end_time_ms}")
        ...
        try:
            ...
            for camera_uidd in selected_cameras:
                try:
                    _LOGGER.debug(f"AI analysis: queueing events for camera {camera_uidd}")
                    logger_server = await self.api.get_logger_server(camera_uidd)
                    if not logger_server:
                    ...
                    _LOGGER.debug(f"AI analysis: {len(events) if events else 0} events for camera {camera_uidd}")
                    ...
                    if events:
                        ...
                        _LOGGER.debug(f"AI analysis: {len(filtered_events)} to process after filtering for camera {camera_uidd}")
            ...
            _LOGGER.info(f"AI analysis: total events to process: {total_events}")
            ...
            if total_events == 0:
                _LOGGER.info("AI analysis: no events found to process")
                self._ai_progress[task_id].update({
            ...
            # Process events
            _LOGGER.debug(f"AI analysis: processing {total_events} events")
            for i, (event, camera_uidd, logger_server) in enumerate(all_events):
                try:
                    _LOGGER.debug(f"AI analysis: event {i+1}/{total_events} from {camera_uidd}")
                    ...
                    if quota_status["daily_remaining"] <= 0:
                        _LOGGER.warning(f"AI analysis: daily quota exhausted after {total_processed} events")
                        self._ai_progress[task_id].update({
                    ...
                    if quota_status["circuit_breaker_active"]:
                        _LOGGER.info(f"AI analysis: circuit breaker active; waiting {quota_status['circuit_breaker_remaining']}s")
                        await asyncio.sleep(quota_status["circuit_breaker_remaining"] + 1)
                        continue
                    
                    if quota_status["minute_remaining"] <= 0:
                        _LOGGER.debug("AI analysis: minute quota exhausted; waiting 60s")
                        await asyncio.sleep(60)
                        continue
                    ...
                    _LOGGER.debug(f"AI analysis: downloading thumbnail for {event_id}")
                    image_data = await self.api.download_event_thumbnail(logger_server, camera_uidd, event_id)
                    if not image_data:
                        _LOGGER.debug(f"AI analysis: no thumbnail for event {event_id}")
                        continue

                    _LOGGER.debug(f"AI analysis: requesting Gemini description for {event_id}")
                    description = await self.get_gemini_description(image_data, api_key)
                    description = self._clean_description(description)
                    
                    if description:
                        _LOGGER.debug(f"AI analysis: description ready for {event_id}")
                        descriptions[event_id] = {
                            'description': description, 'uidd': camera_uidd,
                            'logger_server': logger_server, 'startt': event.get('startt')
                        }
                        
                        # Defer writes; will flush in batches and finalize at end
                        total_processed += 1
                        self._ai_progress[task_id].update({"processed": total_processed})
                    else:
                        _LOGGER.debug(f"AI analysis: empty description for {event_id}")

                except Exception as e:
                    _LOGGER.warning(f"AI analysis: failed processing event {event.get('alert', 'unknown')}: {e}")
                    continue
            ...
            try:
                ...
                _LOGGER.warning(f"Final save of descriptions failed: {e}")
            _LOGGER.info(f"AI analysis completed: {total_processed} events processed successfully")
            self._ai_progress[task_id].update({
