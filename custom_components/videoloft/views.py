...
class AIAnalysisView(HomeAssistantView):
    ...
    async def post(self, request: web.Request) -> web.Response:
        """Handle POST request to start AI analysis."""
        try:
            data = await request.json()
            _LOGGER.info("Received AI analysis request: %s", data)
            ...
            if not start_date or not end_date:
                _LOGGER.error("Start or end date not provided")
            ...
            _LOGGER.info(f"Starting AI analysis for {len(cameras_to_process)} cameras")
            ...
            processed_events = len(cameras_to_process) * 10  # Rough estimate
            return web.json_response({
                "success": True,
...
class AISearchView(HomeAssistantView):
    ...
    async def post(self, request: web.Request) -> web.Response:
        """Handle POST request to search events."""
        try:
            data = await request.json()
            _LOGGER.info("Received AI search request: %s", data)
            ...
            if not query:
                _LOGGER.error("Search query not provided")
            ...
            for event_id, info in descriptions.items():
                ...
                if query_lower in description.lower():
                    ...
                    event_result = {
                        ...
                        "thumbnail": f"https://{info.get('logger_server', '')}/alertthumb/{camera_uidd}/{event_id}/{await self.get_thumbnail_token(coordinator)}"
                    }
            ...
            _LOGGER.info(f"Found {len(matching_events)} matching events for query '{query}'")
